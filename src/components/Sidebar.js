...
const Sidebar = ({ 
  ...
  // Sync with parent collapsed state for desktop
  useEffect(() => {
    if (!mobile) {
  ...
  // Menu configuration with categories based on user roles
  const menuConfig = {
    admin: [
      {
        category: "MENU UTAMA",
  ...
  const toggleSidebar = () => {
    ...
    // Notify parent component for desktop sidebar
  ...
  // Mobile sidebar always shows full width (not collapsed)
  ...
  return (
    <aside className={`
      ...
      ${mobile ? 'overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent' : 'overflow-y-auto'}
      scroll-smooth
    `}>
      ...
      <nav className="flex-1 py-2 sm:py-3">
        <div className="px-2 sm:px-3">
          {menuSections.map((section, sectionIndex) => (
            <div key={section.category} className={sectionIndex > 0 ? 'mt-3 sm:mt-4' : ''}>
              {/* Category Header */}
              {!showCollapsed && (
              ...
              {/* Category Divider for collapsed mode */}
                    ...
                    <li key={item.name} className="relative group">
                      <button
                        ...
                        className={`
                          ...
                          ${mobile ? 'min-h-[44px] active:scale-[0.98]' : 'min-h-[40px]'}
                          ${
                            isActive
      ...
      <div className="p-3 sm:p-4 border-t border-white/10 bg-black/10">
        {!showCollapsed ? (
          ...
          <div className="flex justify-center">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-white font-semibold text-xs border-2 border-white/30 shadow-sm"
